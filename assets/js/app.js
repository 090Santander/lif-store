/**
 * validaciones.js - Unificado con lógica de navegación
 */
...
// 2. Helper UI: maneja clases is-invalid y mensajes de error
...
// 3. Manejo centralizado de eventos al cargar el DOM
document.addEventListener('DOMContentLoaded', () => {
  ...
  formReg?.addEventListener('submit', (e) => {
    ...
    if (v1 && v2 && v3 && v4 && v5 && v6) {
      ...
      formReg.reset();
      window.location.href = 'login.html'; // Redirige tras validar con éxito
    }
  });
  ...
  formLogin?.addEventListener('submit', (e) => {
    ...
    const v2 = validarCampo(pass, pass?.value.length >= 4 && pass?.value.length <= 10, 'La contraseña debe tener entre 4 y 10 caracteres', 'login-password-error');

    if (v1 && v2) {
      alert(`👋 ¡Bienvenido de nuevo! Sesión iniciada como: ${email.value}`);
      window.location.href = 'index.html'; // Redirige tras validar con éxito
    }
  });
